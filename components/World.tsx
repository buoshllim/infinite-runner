...
const Cabin: React.FC<{ x: number, y: number, z: number }> = ({ x, y, z }) => {
    return (
        <group position={[x, y, z]}>
             ...
             <mesh position={[0, 11, 0]} rotation={[0, Math.PI/4, 0]} castShadow><coneGeometry args={[3.5, 2.5, 4]} /><meshStandardMaterial color="#451a03" /></mesh>
    ...
    return (
        <group position={[x, y + 1.2, z]} rotation={[0.3, rotation, 0.4]}>
             ...
             <group position={[0, 0, -2]} rotation={[-0.5, 0, 0]}>
                 ...
                 <mesh position={[0.4, 0, -4.5]} rotation={[0,0,Math.PI/2]}><cylinderGeometry args={[0.1, 0.1, 0.2]} /><meshStandardMaterial color="#111" /></mesh>
...
const Chicken: React.FC<{ x: number, y: number, z: number }> = ({ x, y, z }) => {
    ...
    return <group ref={ref}><mesh castShadow position={[0, 0.3, 0]}><sphereGeometry args={[0.3, 8, 8]} /><meshStandardMaterial color="#fff" /></mesh><mesh position={[0, 0.5, 0.2]} rotation={[Math.PI/2,0,0]}><coneGeometry args={[0.05, 0.1, 8]} /><meshStandardMaterial color="#f59e0b" /></mesh><mesh position={[0, 0.6, 0]}><boxGeometry args={[0.05, 0.1, 0.1]} /><meshStandardMaterial color="#dc2626" /></mesh></group>
};
